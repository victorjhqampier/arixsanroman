                ...
                <form id="per-form-base">
                    <div class="form-row">
                        <div class="form-group input-group-lg col-md-4">
                            ...
                            <input type="hidden" class="d-none" id="persona-id" name="txtpesonaid" readonly />
...
<script type="text/javascript">
	...
	$("#btn-enviar-peradd").click(function () {
        if($("#per-form-base").valid()){
            let request = arixshell_upload_datos('restinventario/proveedores_post_add', $('#per-form-base').serialize());
            
            if(request.status==true){
                let data = $('#per-form-base #per-dni').val()+' - '+$('#per-form-base #per-names').val()+', '+$('#per-form-base #per-lastname').val()+' '+$('#per-form-base #per-firstname').val();
                $('#per-form-base-result tbody').html('<tr><td>'+data+'</td></tr>');
                arixshell_write_cache_serial("e0x005477arixNewUser",request.ids,data);//Pide 1= nombre clave de identificacion 2: (id)= alguna informacion y 3:(data) alguna descripcion       
    ...
    $("#per-form-base #per-dni").blur(function(){
        let dni = request = $(this).val();
        if(request.length == 8 || request.length==11){           
            request = arixshell_upload_datos('restinventario/proveedores_duplicate_check', 'txtdata='+arixshell_verify_data(request)+'&');
            if(request.status==false){
                $("#per-form-base #per-dni").addClass('is-valid');
                if(request.extra != false){
                    $("#per-form-base #persona-id").val(request.extra.axid);
                    $("#per-form-base #per-email").val(request.extra.correo);
                    $("#per-form-base #per-address").val(request.extra.direccion);
                    $("#per-form-base #per-firstname").val(request.extra.materno);
                    $("#per-form-base #per-borndate").val(request.extra.nacimiento);
                    $("#per-form-base #per-names").val(request.extra.nombres);
                    $("#per-form-base #per-lastname").val(request.extra.paterno);                    
                    $("#per-form-base #per-phone").val(request.extra.telefono);
                    $("#per-form-base #per-type").val(request.extra.tipo);
                    $("#per-form-base #per-sexe").val(request.extra.sexo);
                    arixshell_cargar_obigeo('#per-form-base #per-departament','#per-form-base #per-province','#per-form-base #per-distrite',request.extra.distrito,request.extra.provincia,request.extra.departamento);                    
                }else{
