<?php defined('BASEPATH') OR exit('No direct script access allowed'); ?>
<div class="col-xl-12 col-md-12">
   <div class="toast fade show" role="alert" aria-live="assertive" aria-atomic="true">
      <div class="toast-header">
         <strong class="mr-auto">Sistema de gestion del TUC - Juliaca <small>(Alfa)</small> </strong>
         <small>25 de junio 2021</small>
         <button type="button" class="ml-2 mb-1 close" data-dismiss="toast" aria-label="Close">
         <span aria-hidden="true">&times;</span>
         </button>
      </div>
      <div class="toast-body">
         Ésta es la versión Alfa, el desarrolo de la aplicacion es echa por peticion de la gerencia de transporte y seguridad vial - Juliaca.
         <code>Avance estimado ... 14%</code>
         <hr>
         <div class="progress">
            <div class="progress-bar" role="progressbar" style="width: 17%" aria-valuenow="14" aria-valuemin="0" aria-valuemax="100"></div>
         </div>
      </div>
   </div>
</div>
<script type="text/javascript">
$(document).ready(function(){
    arixshell_iniciar_llaves_locales("#btn_001mpsar","#con_001mpsar");
    arixshell_cargar_botones_menu();
    //arixshell_cargar_boton_buscar('Buscar por DNI');
}); 
</script>
