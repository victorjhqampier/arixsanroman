...
class Serv_ticket extends FPDF{
    function __construct($arrData=[1,2,3]) {
        // 170 == arrarLong * 8
    ...
    public function Header(){   
        ...
        $this->Cell(0, 3, 'REGISTRO DE ENTRADA', 0, 1, 'C');
        $this->SetFont('Courier', '', 6);
        $this->MultiCell(0, 3, "BOLETA DE VENTA ELECTRONICA No. 002022488",  0, 'C');
        $this->Cell(0, 5, 'FECHA DE EMISION: 13/09/2021 12:39:55', 0, 1, 'L');
        $this->MultiCell(0, 2, "PROVEEDOR: 48207109 VICTOR JHAMPIER CAXI MAQUERA",  0, 'L');
        $this->ln(3);    
    }
    public function tableDatos(){
        ...
        $this->Cell(0, 1, '---------------------------------------------------', 0, 1, 'L'); 
        $this->Cell(14, 2, 'CODIGO', 0, 0, 'L');       
        $this->Cell(10, 2, 'PRODUCTO', 0, 0, 'L');
        $this->Cell(8, 2, 'VCTO', 0, 0, 'C'); 
        $this->Cell(6, 2, 'CANT', 0, 0, 'C'); 
        $this->Cell(9, 2, 'P.UNIT', 0, 0, 'C'); 
        $this->Cell(9, 2, 'IMPORTE', 0, 1, 'C');
        $this->Cell(0, 1, '---------------------------------------------------', 0, 1, 'L'); 

        $this->Cell(14, 2, '00207109', 0, 0, 'L');       
        $this->Cell(0, 2, 'NUTRILAC 100-ml', 0, 1, 'L');
        //$this->Cell(25, 3, '', 0, 0, 'C');
        $this->Cell(32, 3, '1924-23-40', 0, 0, 'R'); 
        $this->Cell(6, 3, '1', 0, 0, 'C'); 
        $this->Cell(9, 3, '100.00', 0, 0, 'R'); 
        $this->Cell(9, 3, '1000.00', 0, 1, 'R');
        */
        //$this->ln(); 
        $this->SetFont('Courier', '', 5);
